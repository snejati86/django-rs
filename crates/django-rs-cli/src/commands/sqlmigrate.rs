...
pub fn generate_migration_sql(
    ...
    vec![
        format!(
            "-- SQL for {direction} migration {app_label}.{migration_name}"
        ),
        format!("-- (In a full implementation, this would contain actual SQL statements)"),
        format!(
            "-- Generated by django-rs sqlmigrate for {app_label}/{migration_name}"
        ),
    ]
}
...
impl ManagementCommand for SqlmigrateCommand {
    ...
    async fn handle(
        ...
        let app_label = matches
            ...
            .ok_or_else(|| {
                DjangoError::ConfigurationError("app_label is required".to_string())
            })?;
        let migration_name = matches
